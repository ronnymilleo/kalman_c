 ...
 * for real-time state estimation. The filter supports prediction and update
 * (correction) steps with configurable state dimensions, measurement dimensions,
 * and optional control inputs.
 * 
 * The implementation follows the standard Kalman filter equations:
 * - Prediction: x(k|k-1) = F * x(k-1|k-1) + B * u(k-1)
 * - Update: x(k|k) = x(k|k-1) + K(k) * (z(k) - H * x(k|k-1))
 * 
 ...
 * Kalman filter operation. The structure is opaque to the user and should
 ...
 * - State transition matrix (F): System dynamics model
...
/**
 * @brief Perform Kalman filter prediction step
 * 
 * Executes the prediction phase of the Kalman filter algorithm:
 * - Predicts the state: x(k|k-1) = F * x(k-1|k-1) + B * u(k-1)
 * - Predicts covariance: P(k|k-1) = F * P(k-1|k-1) * F^T + Q
 * 
 ...
 * @pre All required matrices (F, Q) must be set
 * @pre If control is provided, control matrix B must be set
 * 
 * @note Control parameter can be NULL if no control input is used
 * @note Control matrix B is only used if control is provided and controlDim > 0
 * 
 * @warning Returns -1 if filter is not initialized
 * @warning Returns -1 if required matrices are not set
 * 
 * @see kalman_update()
 * @see kalman_set_error_covariance()
 */
int kalman_predict(kalman_t *kf, const double *control);

/**
 * @brief Perform Kalman filter update step (correction)
 * 
 * Executes the update (correction) phase of the Kalman filter algorithm:
 * - Calculates innovation: y = z - H * x(k|k-1)
 * - Computes Kalman gain: K = P * H^T * (H * P * H^T + R)^(-1)
 * - Updates state: x(k|k) = x(k|k-1) + K * y
 * - Updates covariance: P(k|k) = (I - K * H) * P(k|k-1)
 * 
 * @param kf Pointer to initialized kalman_t structure
 ...
 * 
 * @pre Filter must be initialized with kalman_set_error_covariance()
 * @pre All required matrices (H, R) must be set
 * @pre Measurement vector must have correct dimension
 * 
 * @note Measurement dimension must match the configured measurement dimension
 * @note Matrix inversion uses Gaussian elimination with partial pivoting
 * 
 * @warning Returns -1 if filter is not initialized
 * @warning Returns -1 if measurement is NULL
 * @warning Returns -1 if matrix inversion fails (singular matrix)
 * 
 * @see kalman_predict()
 * @see kalman_set_observation_matrix()
 * @see kalman_set_measurement_noise_covariance()
 */
